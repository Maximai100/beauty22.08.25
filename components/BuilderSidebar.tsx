...
import type { LandingPageData, Service, PortfolioImage, Theme, Appointment, Client, ContactData, SocialData, Testimonial } from '../types';
...
interface BuilderSidebarProps {
  ...
  onUpdateHero: (field: 'title' | 'subtitle' | 'cta', value: string) => void;
...
const TabButton: React.FC<TabButtonProps> = ({ tab, icon, children, activeTab, onClick }) => (
    <button
        ...
        activeTab === tab ? 'bg-fuchsia-100 text-fuchsia-800' : 'text-gray-600 hover:bg-gray-200'
        }`}
    >
        {icon}
...
export const BuilderSidebar: React.FC<BuilderSidebarProps> = ({
  ...
  const handleGenerateAbout = async () => {
    if (!ai) {
      addToast("Ошибка: AI сервис не доступен.");
      return;
    }
    setIsGeneratingAbout(true);
    try {
      const prompt = `Ты — профессиональный копирайтер для бьюти-индустрии. Напиши привлекательный и теплый текст для раздела "Обо мне" для бьюти-мастера, основываясь на этом тексте: "${data.about}". Улучши стиль, сделай его более личным и профессиональным. Ответ должен быть только текстом для раздела, без лишних вступлений. Язык: русский.`;
      ...
      onUpdateAbout(response.text.trim());
      addToast("Текст 'Обо мне' обновлен с помощью AI!");
    } catch (error) {
      console.error("Error generating 'about' text:", error);
      addToast("Не удалось сгенерировать текст.");
    } finally {
  ...
  const handleGenerateServiceDescription = async (index: number, serviceName: string) => {
      ...
      try {
          ...
          addToast(`Описание для "${serviceName}" обновлено!`);
  
      } catch (error) {
          console.error("Error generating service description:", error);
          addToast("Не удалось сгенерировать описание.");
      } finally {
  ...
  return (
    <aside className="w-96 bg-white h-screen flex flex-col shadow-lg border-r border-gray-200">
      <div className="px-6 py-4 border-b">
        <h1 className="text-2xl font-bold text-gray-800">Конструктор сайта</h1>
        <p className="text-sm text-gray-500">Настройте вашу страницу</p>
      </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        {activeTab === 'content' && <ContentEditor data={data} onUpdateHero={onUpdateHero} onUpdateAbout={onUpdateAbout} onGenerateAbout={handleGenerateAbout} isGenerating={isGeneratingAbout} />}
       ...
       <div className="p-4 border-t text-xs text-gray-500">
        <button onClick={onReset} className="w-full mb-2 flex items-center justify-center py-2 px-4 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
            <ResetIcon />
            Сбросить настройки
        </button>
        <p className="text-center">Конструктор сайтов v1.3 (с Отзывами)</p>
      </div>
    </aside>
  );
};
...
const InputGroup: React.FC<{ label: string; value: string | number; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; name: string; type?: string; placeholder?: string; }> = ({ label, name, ...props }) => (
  <div>
    ...
    <input id={name} name={name} {...props} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-fuchsia-500 focus:border-fuchsia-500" />
  </div>
);
...
const TextareaGroup: React.FC<{ label: string; value: string; onChange: (e: React.ChangeEvent<HTMLTextAreaElement>) => void; name: string; rows?: number }> = ({ label, name, rows = 5, ...props }) => (
  <div>
    <label htmlFor={name} className="block text-sm font-medium text-gray-700 mb-1">{label}</label>
    <textarea id={name} name={name} {...props} rows={rows} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-fuchsia-500 focus:border-fuchsia-500" />
  </div>
);

const ContentEditor: React.FC<Pick<BuilderSidebarProps, 'data' | 'onUpdateHero' | 'onUpdateAbout'> & { onGenerateAbout: () => void; isGenerating: boolean; }> = ({ data, onUpdateHero, onUpdateAbout, onGenerateAbout, isGenerating }) => (
  <div className="space-y-6">
    <div>
      <h3 className="text-lg font-semibold text-gray-800 mb-3">Главный экран</h3>
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
          <h3 className="text-lg font-semibold text-gray-800">Секция "Обо мне"</h3>
          {ai && (
             <button onClick={onGenerateAbout} disabled={isGenerating} className="group flex items-center space-x-1.5 px-2 py-1 rounded-md bg-fuchsia-50 hover:bg-fuchsia-100 transition-colors disabled:bg-gray-100 disabled:cursor-wait">
                {isGenerating ? <Spinner className="h-4 w-4 text-fuchsia-600" /> : <MagicWandIcon className="h-4 w-4 text-fuchsia-600" />}
                <span className="text-xs font-semibold text-fuchsia-700">{isGenerating ? 'Генерация...' : 'Улучшить текст'}</span>
            </button>
          )}
      </div>
      <TextareaGroup label="Описание" name="about" value={data.about} onChange={e => onUpdateAbout(e.target.value)} />
    </div>
  </div>
);
...
const ServiceEditor: React.FC<{ services: Service[], onUpdateServices: (services: Service[]) => void, onGenerateDescription: (index: number, name: string) => void; generatingIndex: number | null; }> = ({ services, onUpdateServices, onGenerateDescription, generatingIndex }) => {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-lg font-semibold text-gray-800">Управление услугами</h3>
      {services.map((service, index) => (
        <div key={service.id} className="p-4 border rounded-md space-y-3 bg-gray-50 relative">
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
                <label htmlFor={`s_desc_${index}`} className="block text-sm font-medium text-gray-700">Описание</label>
                {ai && (
                    <button onClick={() => onGenerateDescription(index, service.name)} disabled={generatingIndex === index} className="group flex items-center space-x-1 text-xs font-medium text-fuchsia-600 hover:text-fuchsia-800 disabled:opacity-50 disabled:cursor-wait">
                        {generatingIndex === index ? <Spinner className="h-3 w-3" /> : <MagicWandIcon className="h-3 w-3" />}
                        <span>Сгенерировать</span>
                    </button>
                )}
            </div>
            <textarea id={`s_desc_${index}`} name={`s_desc_${index}`} value={service.description} onChange={e => handleUpdate(index, 'description', e.target.value)} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-fuchsia-500 focus:border-fuchsia-500" />
          </div>
      ...
      <button onClick={addService} className="w-full py-2 px-4 bg-fuchsia-600 text-white rounded-md hover:bg-fuchsia-700 transition-colors">Добавить услугу</button>
    </div>
  );
};

const PortfolioEditor: React.FC<{ portfolio: PortfolioImage[], onUpdatePortfolio: (images: PortfolioImage[]) => void, addToast: (message: string) => void }> = ({ portfolio, onUpdatePortfolio, addToast }) => {
    ...
    const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onloadend = () => {
            ...
            addToast("Изображение добавлено!");
        };
    ...
    const removeImage = (id: string) => {
        ...
        addToast("Изображение удалено.");
    };

    return (
        <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-800">Изображения портфолио</h3>
            <div className="grid grid-cols-2 gap-4">
                {portfolio.map(image => (
                    <div key={image.id} className="relative group">
                        <img src={image.url} alt={image.alt} className="rounded-md object-cover aspect-square" />
                        <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                             <button onClick={() => removeImage(image.id)} className="text-white p-2 bg-red-500 rounded-full hover:bg-red-600">
            ...
            />
            <button onClick={() => fileInputRef.current?.click()} className="w-full mt-4 py-2 px-4 bg-fuchsia-600 text-white rounded-md hover:bg-fuchsia-700 transition-colors">
...
const DesignEditor: React.FC<{ currentTheme: Theme, onUpdateTheme: (primary: string, background: string, text: string, card: string) => void }> = ({ currentTheme, onUpdateTheme }) => {
    return (
        <div className="space-y-6">
            <h3 className="text-lg font-semibold text-gray-800">Цветовая схема</h3>
            <p className="text-sm text-gray-600">Выберите готовый шаблон или настройте цвета.</p>
            <div className="grid grid-cols-2 gap-4">
                {THEME_PRESETS.map(preset => (
                    <button key={preset.name} onClick={() => onUpdateTheme(preset.theme.primary, preset.theme.background, preset.theme.text, preset.theme.card)} className="p-2 border rounded-md text-left hover:shadow-md transition-shadow">
                        <div className="flex items-center space-x-2">
                           <div className="w-6 h-6 rounded-full" style={{ backgroundColor: preset.theme.primary }}></div>
                           <span className="text-sm font-medium">{preset.name}</span>
                        </div>
                        <div className="flex space-x-1 mt-2">
                           <div className="w-1/4 h-3 rounded" style={{ backgroundColor: preset.theme.primary }}></div>
                           <div className="w-1/4 h-3 rounded" style={{ backgroundColor: preset.theme.background }}></div>
                           <div className="w-1/4 h-3 rounded" style={{ backgroundColor: preset.theme.text }}></div>
                           <div className="w-1/4 h-3 rounded" style={{ backgroundColor: preset.theme.card }}></div>
                        </div>
                    </button>
                ))}
            </div>
...
const ContactEditor: React.FC<{ contact: ContactData, socials: SocialData, onUpdate: (contact: ContactData, socials: SocialData) => void }> = ({ contact, socials, onUpdate }) => {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h3 className="text-lg font-semibold text-gray-800 mb-3">Контактная информация</h3>
                <div className="space-y-4">
            ...
            <div>
                <h3 className="text-lg font-semibold text-gray-800 mb-3">Социальные сети</h3>
                <div className="space-y-4">
...
const BookingManager: React.FC<{ appointments: Appointment[], onUpdateAppointments: (appointments: Appointment[]) => void, services: Service[], addToast: (message: string) => void }> = ({ appointments, onUpdateAppointments, services, addToast }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!newAppointment.clientName || !newAppointment.date || !newAppointment.time) {
            addToast("Пожалуйста, заполните все поля."); // Or a different color toast for errors
            return;
        }
        ...
        addToast("Новая запись успешно создана!");
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-semibold text-gray-800">Предстоящие записи</h3>
                <button onClick={() => setIsModalOpen(true)} className="py-2 px-4 bg-fuchsia-600 text-white rounded-md hover:bg-fuchsia-700 transition-colors text-sm font-medium">
                    Добавить запись
                </button>
            </div>
            {appointments.length > 0 ? (
                <ul className="space-y-3">
                    {appointments.sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map(app => (
                        <li key={app.id} className="p-3 bg-gray-50 border rounded-md flex justify-between items-start">
                           <div>
                                <p className="font-semibold text-gray-800">{app.clientName}</p>
                                <p className="text-sm text-gray-600">{app.serviceName}</p>
                                <p className="text-sm text-gray-500 mt-1">{new Date(app.date).toLocaleDateString('ru-RU')} в {app.time}</p>
                           </div>
                ...
                <div className="text-center py-8 px-4 border-2 border-dashed rounded-lg">
                    <CalendarIcon />
                    <p className="mt-2 text-sm font-medium text-gray-700">Нет предстоящих записей</p>
                    <p className="mt-1 text-xs text-gray-500">Нажмите "Добавить запись", чтобы создать новую.</p>
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
                        <form onSubmit={handleSubmit}>
                            <div className="p-6">
                               <h3 className="text-xl font-semibold text-gray-800 mb-4">Новая запись</h3>
                               <div className="space-y-4">
                                   <InputGroup label="Имя клиента" name="clientName" value={newAppointment.clientName} onChange={handleInputChange} />
                                   <div>
                                       <label htmlFor="serviceName" className="block text-sm font-medium text-gray-700 mb-1">Услуга</label>
                                       <select id="serviceName" name="serviceName" value={newAppointment.serviceName} onChange={handleInputChange} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-fuchsia-500 focus:border-fuchsia-500 bg-white">
                            ...
                            <div className="bg-gray-50 px-6 py-3 flex justify-end space-x-3 rounded-b-lg">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="py-2 px-4 bg-white border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Отмена</button>
                                <button type="submit" className="py-2 px-4 bg-fuchsia-600 text-white rounded-md hover:bg-fuchsia-700 text-sm font-medium">Сохранить</button>
                            </div>
                        </form>
                    </div>
                </div>
...
const CrmManager: React.FC<{ clients: Client[], onUpdateClients: (clients: Client[]) => void, addToast: (message: string) => void }> = ({ clients, onUpdateClients, addToast }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!editingClient.name) {
            addToast("Имя клиента не может быть пустым.");
            return;
        }
        ...
        if (isEditing && 'id' in editingClient) {
            ...
            addToast("Данные клиента обновлены!");
        } else {
            ...
            addToast("Новый клиент добавлен!");
        }
        handleCloseModal();
    };
    ...
    const handleDelete = (id: string) => {
        ...
        addToast("Клиент удален.");
    };

    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-semibold text-gray-800">База клиентов</h3>
                <button onClick={() => handleOpenModal()} className="py-2 px-4 bg-fuchsia-600 text-white rounded-md hover:bg-fuchsia-700 transition-colors text-sm font-medium">
                    Добавить клиента
                </button>
            </div>
            {clients.length > 0 ? (
                <ul className="space-y-3">
                    {clients.map(client => (
                        <li key={client.id} className="p-3 bg-gray-50 border rounded-md">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="font-semibold text-gray-800">{client.name}</p>
                                    <p className="text-sm text-gray-600">{client.email} | {client.phone}</p>
                                    <p className="text-sm text-gray-500 mt-1 italic">Заметки: {client.notes}</p>
                                    <p className="text-xs text-gray-400 mt-1">Посещений: {client.visitHistory.length}</p>
                                </div>
                                <div className="flex space-x-2">
                                    <button onClick={() => handleOpenModal(client)} className="text-gray-400 hover:text-blue-500 p-1">
                                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg>
                                    </button>
                                    <button onClick={() => handleDelete(client.id)} className="text-gray-400 hover:text-red-500 p-1">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" /></svg>
                                    </button>
                                </div>
                            </div>
                 ...
                 <div className="text-center py-8 px-4 border-2 border-dashed rounded-lg">
                    <CrmIcon />
                    <p className="mt-2 text-sm font-medium text-gray-700">Клиентская база пуста</p>
                    <p className="mt-1 text-xs text-gray-500">Нажмите "Добавить клиента", чтобы создать карточку.</p>
                </div>
            )}
             ...
             {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
                        <form onSubmit={handleSubmit}>
                            <div className="p-6">
                               <h3 className="text-xl font-semibold text-gray-800 mb-4">{isEditing ? 'Редактировать клиента' : 'Новый клиент'}</h3>
                               <div className="space-y-4">
                            ...
                            <div className="bg-gray-50 px-6 py-3 flex justify-end space-x-3 rounded-b-lg">
                                <button type="button" onClick={handleCloseModal} className="py-2 px-4 bg-white border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Отмена</button>
                                <button type="submit" className="py-2 px-4 bg-fuchsia-600 text-white rounded-md hover:bg-fuchsia-700 text-sm font-medium">Сохранить</button>
                            </div>
                        </form>
                    </div>
...
const TestimonialEditor: React.FC<{ testimonials: Testimonial[], onUpdateTestimonials: (testimonials: Testimonial[]) => void, addToast: (message: string) => void }> = ({ testimonials, onUpdateTestimonials, addToast }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!editingTestimonial.clientName || !editingTestimonial.text) {
            addToast("Имя клиента и текст отзыва не могут быть пустыми.");
            return;
        }
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-semibold text-gray-800">Управление отзывами</h3>
                <button onClick={() => handleOpenModal()} className="py-2 px-4 bg-fuchsia-600 text-white rounded-md hover:bg-fuchsia-700 transition-colors text-sm font-medium">
                    Добавить отзыв
                </button>
            </div>
            {testimonials.length > 0 ? (
                <ul className="space-y-3">
                    {testimonials.map(testimonial => (
                        <li key={testimonial.id} className="p-3 bg-gray-50 border rounded-md">
                            <div className="flex justify-between items-start">
                                <div>
                                ...
                                <div className="flex space-x-2">
                                    <button onClick={() => handleOpenModal(testimonial)} className="text-gray-400 hover:text-blue-500 p-1">
                                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg>
                                    </button>
                                    <button onClick={() => handleDelete(testimonial.id)} className="text-gray-400 hover:text-red-500 p-1">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" /></svg>
                                    </button>
                                </div>
                            </div>
                 ...
                 <div className="text-center py-8 px-4 border-2 border-dashed rounded-lg">
                    <TestimonialIcon />
                    <p className="mt-2 text-sm font-medium text-gray-700">Отзывов пока нет</p>
                    <p className="mt-1 text-xs text-gray-500">Нажмите "Добавить отзыв", чтобы создать первый.</p>
                </div>
            )}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
                        <form onSubmit={handleSubmit}>
                            <div className="p-6">
                               <h3 className="text-xl font-semibold text-gray-800 mb-4">{isEditing ? 'Редактировать отзыв' : 'Новый отзыв'}</h3>
                               <div className="space-y-4">
